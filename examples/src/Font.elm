...
text =
    [ "ABCDEFGHIJKLMNOPQRSTUVWXYZ" |> words red |> rotate 0.005
    , "abcdefghijklmnopqrstuvwxyz" |> words red
    , "0123456789" |> words red
    , "!\"#$%&'()*+,-./" |> words red
    , ":;<=>?[\\]^_`{|}~" |> words red
    , "1234567890" |> words purple
    ]
        ++ (text1 |> List.map (words blue))
        |> List.indexedMap (\i a -> a |> moveDown (i * 16 |> toFloat))
        |> (\l -> l |> group |> moveUp (List.length l |> toFloat |> (*) 8))
...
text1 =
    ...
    , "^_`abcdefghijklmnopqrstuvwxyz{|}~⌂ÇüéâäàåçêëèïîìÄÅÉæÆô"
    , "öòûùÿÖÜ¢£¥₧ƒáíóúñÑªº¿⌐¬½¼¡«»░▒▓│┤╡╢╖╕╣║╗╝╜╛┐└┴┬├─┼"
